...
Class InstagramFeedTimeLineScrape
{
	...
	public function Process($fetch_media_item_cursor = false)
	{
		...
		if ($fetch_media_item_cursor) {
			...
			$grabjson = InstagramHelper::GetStringBetween($access['body'], "window.__additionalDataLoaded('feed',", ");</script>");
			if (!$grabjson) {
				return [
					'status' => false,
					'response' => 'no_feed'
			...
			$response['data'] = json_decode($grabjson,true);
		}

		if ($response['data']['user']['edge_web_feed_timeline']['edges'] != null) {		
