	...
	public function Run()
	{
		...
		while (true) {
			...
			foreach ($FeedList as $key => $post) {
				...
				if ($like_comment == 'y') 
				{
					echo "[•] Membaca Komentar post : {$post['id']}".PHP_EOL;

					$comments = $Working->GetComment($post['code']);
					if (!$comments) 
					{
						echo "[•] Tidak ada komentar pada post : {$post['id']}".PHP_EOL;
						continue;
					}
					...
					foreach ($comments as $comment) 
					{
						...
						echo "[•] Proses Like Comment {$comment['id']}".PHP_EOL;

						$process_comment = $Working->LikeComment($comment);

						if ($process_comment['status'] != false) {
							
							echo "[+++] Sukses Like Komentar {$comment['id']}".PHP_EOL;
							echo "[•] Time : ".date('d-m-Y H:i:s').PHP_EOL;
							echo "[•] Delay {$delay}".PHP_EOL;
							...
							echo "[---] Gagal Like Komentar {$comment['id']}".PHP_EOL;
							echo "[•] Response : {$process_comment['response']}".PHP_EOL;			
							echo "[•] Time : ".date('d-m-Y H:i:s').PHP_EOL;
						}
			...
			{
				echo "[•] Tidak ditemukan Post terbaru, Coba lagi setelah {$delayfeed} detik".PHP_EOL;
				sleep($delayfeed);
